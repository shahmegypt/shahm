...
alter table public.profiles
  ...
  check (patient_condition is null or char_length(trim(patient_condition)) between 2 and 500);

alter table public.trips
  add column if not exists scheduled_at timestamptz;

-- Existing trips from the previous schema did not have appointments. Keep them
-- valid by treating their creation time as the historical schedule.
update public.trips
...
alter table public.trips
  ...
  check (scheduled_at is not null and scheduled_at >= created_at - interval '1 minute');

create index if not exists idx_trips_pending_scheduled_at
  on public.trips (scheduled_at)
  where status = 'pending';

-- Replace the public pending-trip projection so volunteers receive only
-- nearby requests, plus the minimum patient/scheduling information needed
-- before acceptance. Exact addresses and phone numbers remain protected.
...
begin
  ...
  if not exists (
    ...
    where id = v_user_id and role = 'volunteer' and is_active
  ) then
...
-- Keep the old broad SELECT privilege out of the client. Volunteers now
-- use get_pending_trips_nearby instead of reading pending trips directly.
revoke select on public.trips from authenticated;
-- Requesters/assigned volunteers can still read the public trip projection.
-- Pending volunteer discovery is handled only by get_pending_trips_nearby.
...
grant select (id, requester_id, volunteer_id, origin_area_label, destination_area_label, status, requester_relation, created_at, accepted_at, completed_at, scheduled_at) on public.trips to authenticated;

-- The client can still select its own trip, but pending volunteer reads are
-- intentionally performed through the proximity RPC above.
revoke all on function public.get_pending_trips_nearby(double precision, double precision, double precision) from public, anon, authenticated;
...
-- Create-trip proxy: include scheduled time and enforce it server-side.
...
begin
  ...
  if not exists (
    ...
    where id = p_requester_id and role = 'requester' and is_active
  ) then
...
-- Direct RPC is service-role only in this architecture, but keep it complete
...
begin
  ...
  if not exists (select 1 from profiles where id = v_requester_id and role = 'requester' and is_active) then
    raise exception 'requester role required';
  end if;
  select count(*) into v_completed_count from trips where requester_id = v_requester_id and status = 'completed';
  if v_completed_count < 3 and exists (select 1 from trips where requester_id = v_requester_id and status in ('pending', 'accepted')) then
    raise exception 'one open trip is allowed until three trips are completed';
  end if;
  insert into trips (
    requester_id, origin_area_label, destination_area_label, requester_relation,
...
-- Acceptance must receive the volunteer's current location and repeat the
-- 20-km check on the server. This prevents bypassing the UI.
...
begin
  ...
  update trips
  set status = 'accepted', volunteer_id = v_volunteer_id, accepted_at = now()
...
-- Return the same protected patient/scheduling fields when the volunteer
-- refreshes an already accepted trip.
...
-- Explicitly expose the new profile fields only through the user's own row;
-- volunteer patient data comes from the protected RPCs above.
grant select (id, first_name, phone_number, role, verification_status, is_active, created_at, patient_age, patient_condition)
  on public.profiles to authenticated;

-- A requester may set the patient profile fields only once. Existing values
-- cannot be changed by the client after registration.
...
as $$
begin
  if auth.role() <> 'service_role' and old.id = auth.uid() then
    if old.patient_age is not null and new.patient_age is distinct from old.patient_age then
      raise exception 'patient age can only be provided once during registration';
    end if;
    if old.patient_condition is not null and new.patient_condition is distinct from old.patient_condition then
...
for each row execute function public.prevent_patient_profile_mutation();

-- Remove the obsolete old function signatures so a client cannot accidentally
-- bypass the new scheduling/distance contract.
